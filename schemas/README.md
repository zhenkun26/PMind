...
- `handoff-adapter-local-execution-receipt-v0.yaml`：覆盖真实 local-file-only attempt、十九文件绑定、exact payload digest、幂等 committed 结果和 provider/credential/network/process/cost false 边界；
...
- `handoff-adapter-implementation-attestation-v0.yaml`：覆盖精确十四文件链、实现审核 provenance、Profile/observed effect 差异、provider contract-test 证据和 false runtime/executable/dispatch 边界；
- `handoff-adapter-effect-authorization-confirmation-receipt-v0.yaml`：覆盖精确十三文件链、confirm/modify/reject、具名 effect grants、费用/生产数据派生和 false executable/dispatch 边界；
- `handoff-adapter-effect-authorization-proposal-v0.yaml`：覆盖精确十二文件链、Profile true-effect 集合、费用/生产数据披露、pending 选择和零 effect/dispatch authorization 边界；
...
- `handoff-payload-data-attestation-v0.yaml`：覆盖精确十一文件链、完整 Payload 审核 provenance、个人数据/密钥事实、与已选 Profile 策略派生的兼容性和零授权边界；
...
只读重放十一文件已确认 Selection 链，再校验完整 Payload 审核 provenance、事实与所选 Profile 数据策略的派生兼容性：
...
只读重放十二文件兼容 Attestation 链，再校验 exact true-effect 集合、费用/生产数据披露和 pending 零授权 Proposal：
...
只读重放十三文件 Effect Proposal 链，再校验 exact 用户选择、具名 effect grants 与不可执行/未 dispatch 门禁：
...
只读重放十四文件 confirmed effect 链，再校验实现审核 provenance、observed effects 与 provider contract-test 证据声明：
...
只读重放十五文件 compatible implementation 链，再校验运行配置、凭据引用、提交的 provider 健康证据和 retention/export/purpose 声明：

```sh
ruby scripts/preview_handoff_adapter_runtime_readiness_attestation.rb path/to/new-session.yaml path/to/draft-package.yaml path/to/compilation-proposal.yaml path/to/compilation-confirmation.yaml path/to/final-package.yaml path/to/handoff-proposal.yaml path/to/handoff-confirmation.yaml path/to/handoff-envelope.yaml path/to/adapter-profile.yaml path/to/adapter-selection-proposal.yaml path/to/adapter-selection-confirmation.yaml path/to/payload-data-attestation.yaml path/to/adapter-effect-authorization-proposal.yaml path/to/adapter-effect-authorization-confirmation.yaml path/to/adapter-implementation-attestation.yaml path/to/adapter-runtime-readiness-attestation.yaml
```

只读重放十六文件 ready runtime 链，再校验 exact payload/Adapter/destination、确定性幂等、有效期、费用上限与 pending 零 dispatch Proposal：

```sh
ruby scripts/preview_handoff_adapter_dispatch_proposal.rb path/to/new-session.yaml path/to/draft-package.yaml path/to/compilation-proposal.yaml path/to/compilation-confirmation.yaml path/to/final-package.yaml path/to/handoff-proposal.yaml path/to/handoff-confirmation.yaml path/to/handoff-envelope.yaml path/to/adapter-profile.yaml path/to/adapter-selection-proposal.yaml path/to/adapter-selection-confirmation.yaml path/to/payload-data-attestation.yaml path/to/adapter-effect-authorization-proposal.yaml path/to/adapter-effect-authorization-confirmation.yaml path/to/adapter-implementation-attestation.yaml path/to/adapter-runtime-readiness-attestation.yaml path/to/adapter-dispatch-proposal.yaml
```

只读重放十七文件 pending dispatch 链，再校验 exact 用户选择、费用上限授权和零执行 Receipt：
...
仅对当前时间窗内、ready、local-file-only、zero-cost 的十九文件链，在与仓库和来源隔离的现存本地根目录中原子发布 exact Envelope 与 Execution Receipt：
