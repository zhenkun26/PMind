...
成功不证明 Adapter 已实现、已配置、已 dispatch、已接收或产生结果，也不证明用户身份、外部事实或 PMind 商业效果。后续必须运行 [Handoff Payload Data Attestation v0](handoff-payload-data-attestation-v0.md)；兼容性通过后再进入已实现的零授权 [Adapter Effect Authorization Proposal v0](handoff-adapter-effect-authorization-proposal-v0.md)，真实 Adapter 与 dispatch 继续后置。
