...
- 已新增 provider-neutral Adapter Effect Authorization Proposal v0：十三文件只读 preview 只接受 compatible Payload Data Attestation，精确枚举所选 Profile 的全部 true effects，并披露费用、生产数据访问和 retention/export/purpose 尚未核验。Proposal 固定 pending、未保存用户选择、空授权集和 false dispatch；零 effect 也不能跳过未来 dispatch 确认。下一边界是独立 Effect Authorization Confirmation Receipt，仍不得实现真实 Adapter 或 dispatch。
