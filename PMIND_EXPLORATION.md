...
- 已新增 Clarification Confirmation Receipt v0、四文件只读预演和 confirmed-only revision creator：确认回执用字节级摘要绑定原 Session、Answer Receipt 与 Proposal，区分确认、修改和拒绝；只有明确确认可在新路径生成带 lineage 的 `0600` Session 文件，永不覆盖输入，也不推导高风险授权。
...
- 已新增 provider-neutral Handoff Adapter Capability Profile v0 与 Adapter Selection Proposal v0：Profile 明确交付方式、回执、幂等、重试、数据/成本策略、七类副作用和逐项未来授权要求；十文件只读 preview 会先重跑完整 Envelope lineage，再绑定 exact Envelope/Profile 字节并展示 pending 三选一文案。Profile 不是实现，Proposal 固定未选择、未 dispatch、零外部效果/高风险授权。探索同时发现 Envelope 只有整体数据分类，不能证明内嵌 Package 的个人数据或密钥兼容性，因此两项保持 unknown，真实 dispatch 前必须独立审核。
- 已新增 Handoff Adapter Selection Confirmation Receipt v0：十一文件只读 preview 会在同一次来源重放中校验全部十份字节摘要、Envelope/Profile/Proposal 身份、确认原文摘要、时间和数据策略。`confirmed` 只记录选中精确 Profile，修改与拒绝保持未选择；所有状态都固定未 dispatch、零外部效果/高风险授权、零副作用授权。个人数据与密钥兼容性仍为 unknown；下一边界是 provider-neutral Payload Data Attestation，不得直接 dispatch。
- 已新增 provider-neutral Handoff Payload Data Attestation v0：十二文件只读 preview 会先重放 confirmed Adapter Selection 的完整十一文件链，再校验完整 Envelope payload 的 manual/automated/hybrid 审核 provenance、个人数据/密钥受控类别和所选 Profile 策略，确定兼容或阻断。预演器不扫描 Payload，不保存原始命中内容；兼容结果仍为零 dispatch/副作用授权，不兼容结果必须阻断。
- 已新增 provider-neutral Adapter Effect Authorization Proposal v0：十三文件只读 preview 只接受 compatible Payload Data Attestation，精确枚举所选 Profile 的全部 true effects，并披露费用、生产数据访问和 retention/export/purpose 尚未核验。Proposal 固定 pending、未保存用户选择、空授权集和 false dispatch；零 effect 也不能跳过未来 dispatch 确认。
...
- 已新增 Adapter Dispatch Confirmation Receipt v0：十八文件只读 preview 将 confirmed/modify/reject 绑定到 exact Dispatch Proposal 和全部十七份来源。confirmed 只授权 exact payload/destination/idempotency/time/limits/effects/stops，并在 cost effect 时授权 exact fixed-point ceiling；三态都固定 effects 不可执行、Adapter/provider 未触达、dispatch 未尝试、无 delivery/write/cost。下一边界是 Service execution request / preflight，而不是真实 provider 调用。
- 已新增 Adapter Dispatch Execution Preflight v0：十九文件只读 preview 只接受 confirmed exact Dispatch Receipt，绑定全部十八份来源与 submitted validity/credential/health/destination/idempotency/effect-scope/fixed-point cost evidence，确定性派生 ready/blocked 和 canonical active stops。preview 不执行 live check、不预留幂等、不启动 Adapter/provider/dispatch；blocked 终止执行路径，ready 也只允许未来受控 Service。真实执行需要另行明确 provider/runtime/credential/write/cost 授权。
- 已新增 Adapter Local Reference Execution v0：只接受当前时间仍有效、ready、`local_file + local_digest + local_file_write`、zero-cost、无凭据/provider 的十九文件精确链，以及与仓库/来源隔离的显式本地根目录。执行器用锁目录预留，在同一根目录构建 `0700` 临时 bundle，以 `0600` 写入 exact Envelope 和不可变 Execution Receipt，重检来源/时间后原子 rename；最终目录永不覆盖，相同幂等目标必须逐字段和逐字节验证后零写入复用。真实记录仅包含本地写入/attempt，provider/credential/network/process/cost 均为 false。生产 provider executor、真实交付和效果证据仍未实现或授权。
...
确认回执契约位于 docs/product/clarification-confirmation-receipt-v0.md；先用 ruby scripts/preview_clarification_confirmation.rb SESSION RECEIPT PROPOSAL CONFIRMATION 校验精确文件绑定和用户选择。只有 confirmed Receipt 才可运行 ruby scripts/create_clarification_revision.rb SESSION RECEIPT PROPOSAL CONFIRMATION OUTPUT；OUTPUT 必须是不存在的新路径，创建仍不代表高风险授权。
...
更新：用户对 exact Dispatch Proposal 的选择必须写入独立 Receipt，再用 ruby scripts/preview_handoff_adapter_dispatch_confirmation.rb SESSION_REVISION DRAFT_PACKAGE COMPILATION_PROPOSAL COMPILATION_CONFIRMATION FINAL_PACKAGE HANDOFF_PROPOSAL HANDOFF_CONFIRMATION ENVELOPE ADAPTER_PROFILE ADAPTER_SELECTION_PROPOSAL ADAPTER_SELECTION_CONFIRMATION PAYLOAD_DATA_ATTESTATION ADAPTER_EFFECT_AUTHORIZATION_PROPOSAL ADAPTER_EFFECT_AUTHORIZATION_CONFIRMATION ADAPTER_IMPLEMENTATION_ATTESTATION ADAPTER_RUNTIME_READINESS_ATTESTATION ADAPTER_DISPATCH_PROPOSAL ADAPTER_DISPATCH_CONFIRMATION 执行十八文件只读确认。confirmed 只授权 exact dispatch 和适用的 exact cost ceiling；仍未启动 Adapter、调用 provider、尝试 dispatch、取得 delivery receipt、写外部系统或产生费用。下一边界是 Service execution request / preflight；不得把 Receipt 当作执行或效果证据。
...
更新：上述 persisted bundle 可用 ruby scripts/verify_handoff_adapter_local_execution_receipt.rb 后接相同十九文件和 EXECUTION_ROOT 做独立只读审计。审计不重新请求执行，不使用当前窗口，只验证首次 `executed_at` 位于原窗口且不早于 exact Preflight；它与执行器幂等复用共用 canonical Receipt seam。下一最小边界是 provider-neutral Execution Receipt verification interface specification，而非真实 provider 调用。
